...
export const fetchResumesForUser = async (email) => {
    const owned = await Resume.find({ owner: email }).select("id title description");
    const shared = await Resume.find({ "shared.email": email }).select("id title description");
...
export const updateResume = async (req, res) => {
    try {
        ...
        if (resume.owner !== userEmail && !resume.shared.some(user => user.email === userEmail)) {
...
export const shareResume = async (req, res) => {
    try {
        ...
        if (!targetUser) {
            return res.status(404).json({ message: "User not found. Cannot share resume with unregistered user." });
        }
        ...
        const isAlreadyShared = resume.shared.some(user => user.email === targetEmail);
        if (!isAlreadyShared) {
            resume.shared.push({
        ...
        let result = await sendInstantEmail(targetEmail, "Resume Shared", `Hello ${targetUser.name},${ownerEmail} has shared ${resume.title} with you. Please check it. Thank you`);
        if(!result.success) {
        ...
        res.status(500).json({ message: "Failed to share resume", error: error.message });
    }
};
...
export const getSharedList = async (req, res) => {
    try {
        ...
        if (!resume) {
            return res.status(404).json({ message: 'Resume not found', sharedUsers: [] });
        }
        ...
        for (let i = 0; i < resume.shared.length; i++) {
            ...
            if (latestUserDetails) {
                ...
                if (sharedUser.name !== latestUserDetails.name || 
        ...
        return res
            .status(500)
            .json({ message: 'Failed to fetch shared users', error: error.message, sharedUsers: [] });
    }
};
...
export const unshareResume = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: "Failed to unshare resume", error: error.message });
    }
};
...
export const loadResumeSocket = async (req, res) => {
    try {
        ...
        // Return resume data with socket connection info
        res.status(200).json({ 
            resume,
            socketEnabled: true,
            message: "Resume loaded. Connect to socket for real-time collaboration." 
        });
    } catch (error) {
        res.status(500).json({ message: "Failed to load resume", error: error.message });
    }
...
export const updateResumeSocket = async (req, res) => {
    try {
        ...
        io.to(id).emit('resume-updated', {   // <— fixed
            updates,
            updatedBy: userEmail,
            timestamp: new Date()
          });
